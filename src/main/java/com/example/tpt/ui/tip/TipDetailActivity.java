...
public class TipDetailActivity extends AppCompatActivity {
    ...
    private final String[] tipTitles = {
            "기숙사 TIP",
            "도서관 TIP",
            "학식 TIP",
            "학생증 TIP",
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnMenu.setOnClickListener(this::showTipMenu);
    }

    // 팁 목록 팝업 메뉴를 보여주는 함수
    private void showTipMenu(View view) {
        PopupMenu popup = new PopupMenu(this, view);

        for (String tip : tipTitles) {
            popup.getMenu().add(tip);
        }

        popup.setOnMenuItemClickListener(item -> {
            String selectedTitle = item.getTitle().toString();

            tvTipTitle.setText(selectedTitle);

            container.removeAllViews();
            addContent(selectedTitle);
    ...
    // 강조된(굵은) 텍스트 뷰를 동적으로 생성하여 추가하는 함수
